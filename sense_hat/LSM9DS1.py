...
# Modified By: Frank Shrestha <frank_shrestha@logictronix.com>
...
_LSM9DS1_ADDRESS_MAG = 0x1C
_LSM9DS1_ADDRESS_ACCELGYRO = 0x6A
...
class LSM9DS1:
    ...
    def __init__(self):
        ...
        if (
            self._read_u8(
                _XGTYPE, _LSM9DS1_REGISTER_WHO_AM_I_XG) != _LSM9DS1_XG_ID
    ...
    def accel_range(self):
        """The accelerometer range.  Must be a value of:
            - ACCELRANGE_2G     - ACCELRANGE_4G     - ACCELRANGE_8G    - ACCELRANGE_16G
        """
    ...
    def mag_gain(self):
        """The magnetometer gain.  Must be a value of:
            - MAGGAIN_4GAUSS    - MAGGAIN_8GAUSS   - MAGGAIN_12GAUS  - MAGGAIN_16GAUSS
        """
    ...
    def mag_gain(self, val):
        assert val in (MAGGAIN_4GAUSS, MAGGAIN_8GAUSS,
                       MAGGAIN_12GAUSS, MAGGAIN_16GAUSS)
    ...
    def gyro_scale(self):
        """The gyroscope scale.  Must be a value of:
            * GYROSCALE_245DPS      * GYROSCALE_500DPS      * GYROSCALE_2000DPS
        """
    ...
    def read_accel_raw(self):
        ...
        buffer = bytearray(self._read_bytes(
            _XGTYPE, _LSM9DS1_REGISTER_OUT_X_L_XL | 0x80, 6))

        raw_x, raw_y, raw_z = struct.unpack_from("<hhh", buffer)
    ...
    def read_mag_raw(self):
        ...
        buffer = bytearray(self._read_bytes(
            _MAGTYPE, _LSM9DS1_REGISTER_OUT_X_L_M | 0x80, 6))
    ...
    def read_gyro_raw(self):
        ...
        # Read the gyroscope
        buffer = bytearray(self._read_bytes(
            _XGTYPE, _LSM9DS1_REGISTER_OUT_X_L_G | 0x80, 6))
    ...
    def read_temp_raw(self):
        ...
        # Read temp sensor
        buffer = bytearray(self._read_bytes(
            _XGTYPE, _LSM9DS1_REGISTER_TEMP_OUT_L | 0x80, 2))
...
class LSM9DS1_I2C(LSM9DS1):
    ...
    def __init__(self, i2c_bus, mag_address=_LSM9DS1_ADDRESS_MAG, xg_address=_LSM9DS1_ADDRESS_ACCELGYRO):

        self.bus = smbus.SMBus(i2c_bus)

        if mag_address in (0x1C, 0x1E) and xg_address in (0x6A, 0x6B):
            self._mag_device = mag_address
            self._xg_device = xg_address

            super().__init__()
        else:
            raise ValueError("address parmeters are incorrect.")

    def _read_u8(self, sensor_type, address):
        device = self._mag_device if (
            sensor_type == _MAGTYPE) else self._xg_device

        return self.bus.read_byte_data(device, address)

    def _read_bytes(self, sensor_type, address, count):
        device = self._mag_device if (
            sensor_type == _MAGTYPE) else self._xg_device

        return self.bus.read_i2c_block_data(device, address | 0x80, count)

    def _write_u8(self, sensor_type, address, val):
        device = self._mag_device if (
            sensor_type == _MAGTYPE) else self._xg_device

        self.bus.write_byte_data(device, address, val)
